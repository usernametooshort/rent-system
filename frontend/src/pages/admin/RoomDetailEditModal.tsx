...
const RoomDetailEditModal: React.FC<RoomDetailEditModalProps> = ({ isOpen, onClose, room }) => {
    ...
    const addApplianceMutation = useMutation({
        mutationFn: async (data: { name: string; compensationPrice: number }) => {
            const res = await client.post(`/rooms/${room.id}/appliances`, data)
            return res.data
        },
        onSuccess: () => {
            toast.success('添加成功')
    ...
    const deleteApplianceMutation = useMutation({
        mutationFn: async (appId: string) => {
            await client.delete(`/rooms/${room.id}/appliances/${appId}`)
        },
        onSuccess: () => {
            toast.success('删除成功')
            queryClient.invalidateQueries({ queryKey: ['admin-rooms'] })
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            await client.post(`/upload/room-image?roomId=${room.id}`, formData, {
